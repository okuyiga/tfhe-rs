...
use crate::shortint::{
    CiphertextNew, CiphertextOpOrder, ClientKey, CompressedCiphertextNew, Parameters,
};
...
impl ShortintEngine {
    ...
    pub fn encrypt<const OP_ORDER: u8>(
        &mut self,
    ...
    pub fn encrypt_compressed<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
    ) -> EngineResult<CompressedCiphertextNew<OP_ORDER>> {
    ...
    pub(crate) fn encrypt_with_message_modulus<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
        message_modulus: MessageModulus,
    ) -> EngineResult<CiphertextNew<OP_ORDER>> {
        let op_order: CiphertextOpOrder = OP_ORDER.try_into().unwrap();

        let (encryption_lwe_sk, encryption_noise) = match op_order {
            CiphertextOpOrder::KeyswitchPBS => (
                &client_key.large_lwe_secret_key,
                client_key.parameters.glwe_modular_std_dev,
            ),
            CiphertextOpOrder::PBSKeyswitch => (
    ...
    pub(crate) fn encrypt_with_message_modulus_compressed<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
        message_modulus: MessageModulus,
    ...
    pub(crate) fn unchecked_encrypt<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
    ) -> EngineResult<CiphertextNew<OP_ORDER>> {
    ...
    pub(crate) fn decrypt_message_and_carry<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        ...
        let lwe_decryption_key = match op_order {
            CiphertextOpOrder::KeyswitchPBS => &client_key.large_lwe_secret_key,
            CiphertextOpOrder::PBSKeyswitch => &client_key.small_lwe_secret_key,
        };
    ...
    pub fn decrypt<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        ct: &CiphertextNew<OP_ORDER>,
    ) -> EngineResult<u64> {
    ...
    pub(crate) fn encrypt_without_padding<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
    ) -> EngineResult<CiphertextNew<OP_ORDER>> {
    ...
    pub(crate) fn encrypt_without_padding_compressed<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
    ) -> EngineResult<CompressedCiphertextNew<OP_ORDER>> {
    ...
    pub(crate) fn decrypt_message_and_carry_without_padding<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        ct: &CiphertextNew<OP_ORDER>,
    ) -> EngineResult<u64> {
    ...
    pub(crate) fn decrypt_without_padding<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        ct: &CiphertextNew<OP_ORDER>,
    ) -> EngineResult<u64> {
    ...
    pub(crate) fn encrypt_native_crt<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
        message_modulus: u8,
    ...
    pub(crate) fn encrypt_native_crt_compressed<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        message: u64,
        message_modulus: u8,
    ...
    pub(crate) fn decrypt_message_native_crt<const OP_ORDER: u8>(
        &mut self,
        client_key: &ClientKey,
        ct: &CiphertextNew<OP_ORDER>,
        basis: u64,
