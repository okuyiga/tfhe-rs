...
impl ServerKey {
    ...
    /// assert_eq!(modulus - msg, three);
    /// ```
    pub fn unchecked_neg<const OP_ORDER: u8>(
        &self,
    ...
    pub fn unchecked_neg_with_z<const OP_ORDER: u8>(
        &self,
        ct: &CiphertextNew<OP_ORDER>,
    ...
    /// assert_eq!(modulus - msg, cks.decrypt(&ct));
    /// ```
    pub fn unchecked_neg_assign<const OP_ORDER: u8>(&self, ct: &mut CiphertextNew<OP_ORDER>) {
        ShortintEngine::with_thread_local_mut(|engine| {
            engine.unchecked_neg_assign(self, ct).unwrap()
        })
    }

    pub fn unchecked_neg_assign_with_z<const OP_ORDER: u8>(
        &self,
    ...
    /// assert_eq!(can_be_negated, true);
    /// ```
    pub fn is_neg_possible<const OP_ORDER: u8>(&self, ct: &CiphertextNew<OP_ORDER>) -> bool {
    ...
    /// assert_eq!(clear_res, modulus - msg);
    /// ```
    pub fn checked_neg<const OP_ORDER: u8>(
        &self,
        ct: &CiphertextNew<OP_ORDER>,
    ...
    /// assert_eq!(clear_res, modulus - msg);
    /// ```
    pub fn checked_neg_assign<const OP_ORDER: u8>(
        &self,
        ct: &mut CiphertextNew<OP_ORDER>,
    ) -> Result<(), CheckError> {
        if self.is_neg_possible(ct) {
    ...
    /// assert_eq!(clear_res, modulus - msg);
    /// ```
    pub fn smart_neg<const OP_ORDER: u8>(
        &self,
        ct: &mut CiphertextNew<OP_ORDER>,
    ) -> CiphertextNew<OP_ORDER> {
    ...
    /// assert_eq!(clear_res, modulus - msg);
    /// ```
    pub fn smart_neg_assign<const OP_ORDER: u8>(&self, ct: &mut CiphertextNew<OP_ORDER>) {
