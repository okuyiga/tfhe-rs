...
use crate::shortint::ciphertext::{
    CiphertextBig, CiphertextNew, CiphertextSmall, CompressedCiphertextBig,
...
impl ClientKey {
    ...
    /// ```rust
    /// use tfhe::shortint::parameters::MessageModulus;
    /// use tfhe::shortint::{CiphertextBig, ClientKey, Parameters};
    ///
    /// // Generate the client key
    /// let cks = ClientKey::new(Parameters::default());
    ///
    ...
    /// assert_eq!(msg, dec);
    /// ```
    pub fn decrypt_message_and_carry<const OP_ORDER: u8>(
        &self,
    ...
    /// assert_eq!(msg, dec);
    /// ```
    pub fn decrypt<const OP_ORDER: u8>(&self, ct: &CiphertextNew<OP_ORDER>) -> u64 {
    ...
    /// assert_eq!(msg, dec);
    /// ```
    pub fn decrypt_message_and_carry_without_padding<const OP_ORDER: u8>(
        &self,
        ct: &CiphertextNew<OP_ORDER>,
    ) -> u64 {
        ShortintEngine::with_thread_local_mut(|engine| {
    ...
    /// assert_eq!(msg % modulus, dec);
    /// ```
    pub fn decrypt_without_padding<const OP_ORDER: u8>(&self, ct: &CiphertextNew<OP_ORDER>) -> u64 {
        ShortintEngine::with_thread_local_mut(|engine| {
    ...
    /// assert_eq!(msg, dec % modulus as u64);
    /// ```
    pub fn decrypt_message_native_crt<const OP_ORDER: u8>(
        &self,
        ct: &CiphertextNew<OP_ORDER>,
        message_modulus: u8,
    ) -> u64 {
