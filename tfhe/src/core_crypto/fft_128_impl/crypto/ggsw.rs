...
use crate::core_crypto::fft_impl::math::decomposition::TensorSignedDecompositionLendingIter;
...
pub type FourierGgswCiphertextView<'a> = FourierGgswCiphertext<&'a [f64]>;
pub type FourierGgswCiphertextMutView<'a> = FourierGgswCiphertext<&'a mut [f64]>;
pub type FourierGgswLevelMatrixView<'a> = FourierGgswLevelMatrix<&'a [f64]>;
pub type FourierGgswLevelMatrixMutView<'a> = FourierGgswLevelMatrix<&'a mut [f64]>;
pub type FourierGgswLevelRowView<'a> = FourierGgswLevelRow<&'a [f64]>;
pub type FourierGgswLevelRowMutView<'a> = FourierGgswLevelRow<&'a mut [f64]>;

impl<C: Container> FourierGgswCiphertext<C> {
    ...
    pub fn as_view(&self) -> FourierGgswCiphertext<&[C::Element]>
    where
        C: AsRef<[C::Element]>,
    {
    ...
    pub fn as_mut_view(&mut self) -> FourierGgswCiphertext<&mut [C::Element]>
    where
        C: AsMut<[C::Element]>,
    {
        FourierGgswCiphertext {
...
impl<C: Container> FourierGgswLevelMatrix<C> {
    pub fn from_container(
        data_re0: C,
        data_re1: C,
        data_im0: C,
        ...
        assert_eq!(polynomial_size.0 % 2, 0);
        let container_len = polynomial_size.0 / 2 * glwe_size.0 * row_count;
        assert_eq!(data_re0.container_len(), container_len);
...
impl<C: Container> FourierGgswLevelRow<C> {
    pub fn from_container(
        data_re0: C,
        data_re1: C,
        data_im0: C,
        ...
        assert_eq!(polynomial_size.0 % 2, 0);
        let container_len = polynomial_size.0 / 2 * glwe_size.0;
        assert_eq!(data_re0.container_len(), container_len);
        assert_eq!(data_re1.container_len(), container_len);
        assert_eq!(data_im0.container_len(), container_len);
        assert_eq!(data_im1.container_len(), container_len);
...
impl<'a> FourierGgswCiphertextMutView<'a> {
    ...
    pub fn fill_with_forward_fourier<Scalar: UnsignedTorus>(
        ...
        let (data_re0, data_re1, data_im0, data_im1) = self.data();

        for (fourier_re0, fourier_re1, fourier_im0, fourier_im1, coef_poly) in izip!(
            data_re0.into_chunks(poly_size / 2),
            data_re1.into_chunks(poly_size / 2),
            data_im0.into_chunks(poly_size / 2),
            data_im1.into_chunks(poly_size / 2),
            coef_ggsw.as_polynomial_list().iter()
        ) {
            // SAFETY: forward_as_torus doesn't write any uninitialized values into its output
            fft.forward_as_torus(
                unsafe { as_mut_uninit(fourier_re0) },
                unsafe { as_mut_uninit(fourier_re1) },
                unsafe { as_mut_uninit(fourier_im0) },
                unsafe { as_mut_uninit(fourier_im1) },
                coef_poly.as_ref(),
            );
        }
    }
...
pub fn add_external_product_assign_scratch<Scalar>(
    ...
    let fourier_scratch =
        StackReq::try_new_aligned::<f64>(glwe_size.0 * polynomial_size.0 / 2, align)?;
    let fourier_scratch_single = StackReq::try_new_aligned::<f64>(polynomial_size.0 / 2, align)?;
...
unsafe fn update_with_fmadd_scalar(
    output_fourier_re0: &mut [MaybeUninit<f64>],
    output_fourier_re1: &mut [MaybeUninit<f64>],
    output_fourier_im0: &mut [MaybeUninit<f64>],
    output_fourier_im1: &mut [MaybeUninit<f64>],
    ggsw_poly_re0: &[f64],
    ggsw_poly_re1: &[f64],
    ggsw_poly_im0: &[f64],
    ggsw_poly_im1: &[f64],
    ...
    if is_output_uninit {
        ...
        for (
            ...
            out_fourier_re0.write(out_re.0);
            out_fourier_re1.write(out_re.1);
            out_fourier_im0.write(out_im.0);
            out_fourier_im1.write(out_im.1);
        }
    } else {
        ...
        for (
            ...
            let mut out_re = f128(out_fourier_re0.assume_init(), out_fourier_re1.assume_init());
            let mut out_im = f128(out_fourier_im0.assume_init(), out_fourier_im1.assume_init());
...
pub unsafe fn update_with_fmadd(
    output_fft_buffer_re0: &mut [MaybeUninit<f64>],
    output_fft_buffer_re1: &mut [MaybeUninit<f64>],
    output_fft_buffer_im0: &mut [MaybeUninit<f64>],
    output_fft_buffer_im1: &mut [MaybeUninit<f64>],
    ...
    for (
        ...
        output_fft_buffer_im0.into_chunks(poly_size / 2),
        output_fft_buffer_im1.into_chunks(poly_size / 2),
        ggsw_row.data_re0.into_chunks(poly_size / 2),
        ggsw_row.data_re1.into_chunks(poly_size / 2),
        ggsw_row.data_im0.into_chunks(poly_size / 2),
        ggsw_row.data_im1.into_chunks(poly_size / 2),
    ) {
