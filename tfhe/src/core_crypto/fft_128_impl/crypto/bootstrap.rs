...
pub struct Fourier128LweBootstrapKey<C> {
    data_re0: C,
    data_re1: C,
    data_im0: C,
    data_im1: C,
...
pub type FourierLweBootstrapKeyView<'a> = Fourier128LweBootstrapKey<&'a [f64]>;
pub type FourierLweBootstrapKeyMutView<'a> = Fourier128LweBootstrapKey<&'a mut [f64]>;

impl<C: Container> Fourier128LweBootstrapKey<C> {
    pub fn from_container(
        ...
        let container_len = input_lwe_dimension.0 * polynomial_size.0 / 2
...
pub type FourierLweBootstrapKeyOwned = Fourier128LweBootstrapKey<ABox<[f64]>>;
