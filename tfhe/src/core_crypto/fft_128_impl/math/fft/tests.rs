...
fn test_roundtrip<Scalar: UnsignedTorus>() {
    ...
    for i in 6..=14 {
        ...
        let mut fourier_re0 = avec![0.0f64; size / 2].into_boxed_slice();
        let mut fourier_re1 = avec![0.0f64; size / 2].into_boxed_slice();
        let mut fourier_im0 = avec![0.0f64; size / 2].into_boxed_slice();
        let mut fourier_im1 = avec![0.0f64; size / 2].into_boxed_slice();

        for x in poly.as_mut().iter_mut() {
            *x = generator.random_uniform();
        }

        let mut mem = GlobalMemBuffer::new(fft.backward_scratch().unwrap());
        let mut stack = DynStack::new(&mut mem);

        fft.forward_as_torus(
            unsafe { as_mut_uninit(&mut fourier_re0) },
            unsafe { as_mut_uninit(&mut fourier_re1) },
            unsafe { as_mut_uninit(&mut fourier_im0) },
            unsafe { as_mut_uninit(&mut fourier_im1) },
            &poly,
        );
        fft.backward_as_torus(
            unsafe { as_mut_uninit(&mut roundtrip) },
            &fourier_re0,
            &fourier_re1,
            &fourier_im0,
            &fourier_im1,
...
fn test_product<Scalar: UnsignedTorus>() {
    ...
    for i in 6..=14 {
        for _ in 0..10 {
            ...
            let mut fourier0_re0 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier0_re1 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier0_im0 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier0_im1 = avec![0.0f64; size / 2].into_boxed_slice();

            let mut fourier1_re0 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier1_re1 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier1_im0 = avec![0.0f64; size / 2].into_boxed_slice();
            let mut fourier1_im1 = avec![0.0f64; size / 2].into_boxed_slice();
            ...
            fft.forward_as_torus(
                unsafe { as_mut_uninit(&mut fourier0_re0) },
                unsafe { as_mut_uninit(&mut fourier0_re1) },
                unsafe { as_mut_uninit(&mut fourier0_im0) },
                unsafe { as_mut_uninit(&mut fourier0_im1) },
                &poly0,
            );
            fft.forward_as_integer(
                unsafe { as_mut_uninit(&mut fourier1_re0) },
                unsafe { as_mut_uninit(&mut fourier1_re1) },
                unsafe { as_mut_uninit(&mut fourier1_im0) },
                unsafe { as_mut_uninit(&mut fourier1_im1) },
                &poly1,
            );
            ...
            fft.backward_as_torus(
                unsafe { as_mut_uninit(&mut convolution_from_fft) },
                &fourier0_re0,
                &fourier0_re1,
                &fourier0_im0,
                &fourier0_im1,
            ...
            for (expected, actual) in izip!(
                ...
                let threshold = Scalar::ONE << (Scalar::BITS - 5);
                let abs_diff = abs_diff(*expected, *actual);
                assert!(
                    abs_diff < threshold,
                    "abs_diff: {abs_diff}, threshold: {threshold}",
