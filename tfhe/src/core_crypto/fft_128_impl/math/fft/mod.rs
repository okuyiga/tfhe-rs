...
fn to_signed_to_f128<Scalar: UnsignedInteger>(x: Scalar) -> f128 {
    if TypeId::of::<Scalar>() == TypeId::of::<u128>() {
        u128_to_signed_to_f128(unsafe { transmute_copy(&x) })
    } else {
...
fn from_torus_f128<Scalar: UnsignedInteger>(x: f128) -> Scalar {
    if TypeId::of::<Scalar>() == TypeId::of::<u128>() {
        unsafe { transmute_copy(&u128_from_torus_f128(x)) }
    } else {
        let mut x = x - f128_floor(x);
...
fn convert_backward_torus<Scalar: UnsignedTorus>(
        ...
        izip!(out_re, out_im, in_re0, in_re1, in_im0, in_im1)
    {
        let in_re = f128(*in_re0 * norm, *in_re1 * norm);
        let in_im = f128(*in_im0 * norm, *in_im1 * norm);
        out_re.write(from_torus_f128(in_re));
        out_im.write(from_torus_f128(in_im));
    }
}
...
impl<'a> Fft128View<'a> {
    ...
    unsafe fn forward_with_conv<Scalar: UnsignedTorus>(
        self,
        fourier_re0: &mut [MaybeUninit<f64>],
        fourier_re1: &mut [MaybeUninit<f64>],
        fourier_im0: &mut [MaybeUninit<f64>],
    ...
    pub fn backward_as_torus<Scalar: UnsignedTorus>(
        self,
        standard: &mut [MaybeUninit<Scalar>],
        fourier_re0: &[f64],
        fourier_re1: &[f64],
        fourier_im0: &[f64],
        fourier_im1: &[f64],
    ...
    unsafe fn backward_with_conv<
        ...
        F: Fn(&mut [MaybeUninit<Scalar>], &mut [MaybeUninit<Scalar>], &[f64], &[f64], &[f64], &[f64]),
    >(
        self,
        standard: &mut [MaybeUninit<Scalar>],
        fourier_re0: &[f64],
